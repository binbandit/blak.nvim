...
SMOKE_RUNTIME_CMD = NVIM_APPNAME=$(SMOKE_NVIM_APPNAME) nvim --headless -u NONE --cmd 'set loadplugins' --cmd '$(SMOKE_RTP)' --cmd 'lua vim.g.blak_config={ui={splash={enabled=false}},mason={automatic_install=false},treesitter={ensure_installed={}}}' --cmd 'runtime init.lua'
...
smoke:
	...
	$(SMOKE_CMD) -c qa
	$(SMOKE_RUNTIME_CMD) -c 'lua dofile("scripts/smoke-pairs.lua")'
	$(COMMANDS_CMD) -c qa
